<script>
    ...
    async function updateRegistration() {
      ...
      if (
        !registration.year || !registration.province || !registration.total_general_national  ||
        registration.total_general_import=== null || registration.total_general_auction === null  || registration.total_general ===null
      ) {
      ...
      if (res.ok) {
        ...
        errorMessage = "⚠️ Datos incorrectos, por favor revisa los valores ingresados.";
      } else if (res.status === 409) {
        errorMessage = "⚠️ Ya existe un registro con estos datos.";
      } else {
        errorMessage = "❌ Error inesperado al actualizar el registro.";
      }
    }
  </script>
